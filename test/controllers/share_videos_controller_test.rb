...
class ShareVideosControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create share_video' do
    assert_difference('ShareVideo.count') do
      post share_videos_url,
           params: { share_video: { description: @share_video.description, title: @share_video.title, url: @share_video.url,
                                    user_id: @share_video.user_id } }
    end
  ...
  test 'should update share_video' do
    patch share_video_url(@share_video),
          params: { share_video: { description: @share_video.description, title: @share_video.title, url: @share_video.url,
                                   user_id: @share_video.user_id } }
